...
export default function NurseScanPage() {
  ...
  // Use unknown so we don't pull the heavy html5-qrcode types into the SSR boundary.
  ...
  async function submitFallback(e: React.FormEvent) {
    ...
    try {
      ...
      const j = await res.json().catch(() => ({}));
      if (!res.ok) {
        setError(j.detail ?? j.error ?? 'Fallback failed.');
        setStage('fallback');
        return;
      }
      setDone(j as DoneResult);
      ...
      <div className="mx-auto max-w-md">
        <Card className="border-slate-800 bg-slate-900 p-5">
          ...
          <p className="mt-1 text-xs text-slate-400">
            ...
            logged with your nurse ID and the current time.
          </p>
          ...
          {stage === 'fallback' && (
            <form onSubmit={submitFallback} className="mt-4 space-y-3">
              <p className="text-xs text-slate-400">
                ...
                patient ID so we can attribute the visit. The fingerprint
                signature is captured by your device biometric prompt (stubbed
                in Phase 0 — passes through for now).
              </p>
              <div>
