...
export default function NewPatientPage() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      setResult({ family_code: j.family_code, patient_id: j.patient_id });
    } catch {
        ...
        <div className="mx-auto max-w-md">
          <Card className="border-slate-800 bg-slate-900 p-6">
            ...
            <p className="mt-2 text-sm text-slate-300">
              Share this code with the patient and family. It is the same code
              for both — they enter it on the home page to access the portal.
            </p>
            <p className="mt-3 select-all rounded bg-slate-800 px-3 py-3 text-center font-mono text-2xl tracking-widest">
              {result.family_code}
            </p>
            <div className="mt-6 flex gap-2">
              <Button
                className="flex-1"
                size="lg"
                onClick={() => router.push(`/er/intake/${result.patient_id}`)}
              >
                Continue to triage
              </Button>
              <Button
            ...
            <section className="space-y-3">
              ...
              <div>
                ...
                <Input
                  ...
                  placeholder="e.g. severe left leg pain, possible fracture"
                  required
                />
              </div>
              <div>
                <Label htmlFor="meds">
                  Medications given in ER (comma-separated)
                </Label>
                <Input
                  id="meds"
                  value={medsGiven}
                  onChange={(e) => setMedsGiven(e.target.value)}
                  placeholder="morphine 5mg IV, paracetamol 1g PO"
                />
              </div>
              <div>
                <Label htmlFor="severity">
                  Triage severity (1=critical, 5=minor)
                </Label>
                <select
                  id="severity"
                  value={severity}
                  onChange={(e) =>
                    setSeverity(e.target.value as '1' | '2' | '3' | '4' | '5')
                  }
                  className="block w-full rounded-md border border-slate-700 bg-slate-800 px-3 py-2 text-sm"
                >
